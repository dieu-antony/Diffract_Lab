...
const DEFAULT_LAYERS: Layer[] = [
  makeDefaultLayer({
    id: "1",
    name: "Au(100) substrate",
    color: "#60a5fa",
    spotSize: 5,
  }),
  makeDefaultLayer({
    id: "2",
    name: "(5×1) domain A [10]",
    color: "#f87171",
    mode: "superlattice",
    matrix: [
      [5, 0],
...
// ── Matrix input ─────────────────────────────────────────────
/** Evaluate a matrix cell expression like "sqrt(3)/2", "√3", "1.5", "0", "-1" */
function evalMatrixExpr(raw: string): number | null {
  const s = raw.trim()
    .replace(/√/g, 'sqrt')
    .replace(/pi/gi, String(Math.PI))
    .replace(/π/g, String(Math.PI));
  // Only allow safe characters: digits, operators, sqrt, whitespace, dot, parens
  if (!/^[0-9+\-*/().sqrt\s]+$/.test(s)) return null;
  try {
    // Replace sqrt(...) with Math.sqrt(...)
    const expr = s.replace(/sqrt\s*\(/g, 'Math.sqrt(');
...
function MatrixInput({ matrix, onChange }: { matrix: Matrix2x2; onChange: (m: Matrix2x2) => void }) {
  // Keep raw string values so the user can type freely mid-expression
  const [raw, setRaw] = useState<Record<string, string>>({
    '00': String(matrix[0][0]),
    '01': String(matrix[0][1]),
    '10': String(matrix[1][0]),
    '11': String(matrix[1][1]),
  });
  // Keep raw in sync when matrix changes externally (preset load)
  const prevMatrix = useRef(matrix);
  useEffect(() => {
    if (matrix !== prevMatrix.current) {
  ...
  const handleChange = (r: 0|1, c: 0|1, value: string) => {
    setRaw(prev => ({ ...prev, [`${r}${c}`]: value }));
  };

  const handleBlur = (r: 0|1, c: 0|1) => {
    const value = raw[`${r}${c}`];
    const parsed = evalMatrixExpr(value);
    if (parsed !== null) {
      // Valid — commit to matrix
      const m: Matrix2x2 = [[...matrix[0]] as [number,number], [...matrix[1]] as [number,number]];
      m[r][c] = parsed;
      onChange(m);
      // Normalise display to the evaluated number
      setRaw(prev => ({ ...prev, [`${r}${c}`]: String(parsed) }));
    } else {
      // Invalid — revert display to last good value
      setRaw(prev => ({ ...prev, [`${r}${c}`]: String(matrix[r][c]) }));
    }
  };
  ...
  return (
    <div>
      ...
      <div className="mt-1 grid grid-cols-2 gap-1.5">
        {cells.map(([r, c, lbl]) => {
          const key = `${r}${c}`;
          const parsed = evalMatrixExpr(raw[key] ?? '');
          const isInvalid = raw[key] !== '' && parsed === null;
          return (
            <div key={lbl} className="flex items-center gap-1">
              ...
              <input
                ...
                className={`w-full bg-gray-800 rounded px-1 py-1 text-xs text-center focus:outline-none border ${
                  isInvalid
      ...
      <p className="mt-1 text-xs text-gray-600 text-center leading-relaxed">
        supports: <span className="text-gray-500">0, 1.5, √3, sqrt(3)/2, -1, pi</span>
      </p>
      <p className="mt-0.5 text-center text-xs text-gray-600 font-mono leading-relaxed">
        [{matrix[0][0].toFixed(4)} {matrix[0][1].toFixed(4)}] · [a₁] = [A₁]<br/>
        [{matrix[1][0].toFixed(4)} {matrix[1][1].toFixed(4)}]   [a₂]   [A₂]
      </p>
    </div>
  );
}
...
export default function LEEDStudio() {
  ...
  const add90DegCopy = useCallback(
    (id: string) => {
      const src = layers.find((l) => l.id === id);
      if (!src) return;
  ...
  const loadPreset = useCallback((label: string) => {
    const preset = RECONSTRUCTION_PRESETS.find((p) => p.label === label);
    if (!preset) return;
    const newLayers = preset.layers.map((pl, i) =>
      makeDefaultLayer({
        ...pl,
        id: makeId(),
        color: pl.color ?? LAYER_COLORS[i % LAYER_COLORS.length],
        matrix: pl.matrix
          ? (pl.matrix.map((r) => [...r]) as Matrix2x2)
          : [
              [1, 0],
              [0, 1],
            ],
  ...
  const loadSurface = useCallback(
    (key: string) => {
      const sp = SURFACE_PRESETS[key];
      if (!sp || !layers[0]) return;
      ...
      <div className="w-72 flex-shrink-0 flex flex-col border-r border-gray-800 bg-gray-900 overflow-hidden">
        ...
        <div className="px-4 py-3 border-b border-gray-800 flex-shrink-0">
          <div className="flex items-center gap-2 mb-2.5">
            ...
            <div>
              <h1 className="text-sm font-bold tracking-widest text-blue-300 uppercase">
                Diffract Lab
              </h1>
              <p className="text-xs text-gray-500">
                Multi-layer pattern simulator
              </p>
            </div>
          </div>
          ...
          {saveMsg && (
            <p className="mt-1.5 text-xs text-center text-green-400">
              {saveMsg}
            </p>
          )}
        </div>
        ...
        <div className="px-4 py-2.5 border-b border-gray-800 flex-shrink-0">
          ...
          <select
            className="mt-1 w-full bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-blue-500"
            value=""
            onChange={(e) => {
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3">
              ...
              {layers.map((layer, idx) => (
                ...
                >
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      upd(layer.id, { visible: !layer.visible });
                    }}
                  ...
                  />
                  <span
                    className={`flex-1 text-xs truncate ${layer.visible ? "text-gray-200" : "text-gray-500"}`}
                  >
                    {layer.name}
                  </span>
                  ...
                  <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      ...
                      title="Duplicate"
                      className="text-gray-500 hover:text-blue-400 px-0.5"
                    >
                      ⎘
                    </button>
                    {layers.length > 1 && (
            ...
            <div className="mt-4">
              ...
              <select
                ...
                value=""
                onChange={(e) => {
                  if (e.target.value) loadSurface(e.target.value);
                }}
              >
                <option value="" disabled>
                  Set substrate to…
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  upd(selectedLayer.id, { name: e.target.value })
                }
            ...
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                onChange={(e) =>
                  upd(selectedLayer.id, { color: e.target.value })
                }
              ...
              <div className="flex gap-1 flex-wrap">
                {LAYER_COLORS.map((c) => (
                  <button
                    key={c}
                    onClick={() => upd(selectedLayer.id, { color: c })}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  upd(selectedLayer.id, { mode: e.target.value as LayerMode })
                }
                className="mt-0.5 w-full bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-xs focus:outline-none focus:border-blue-500"
              >
            ...
            {selectedLayer.mode === "direct" && (
              <>
                <div>
                  ...
                  <select
                    value={selectedLayer.system}
                    onChange={(e) =>
                      upd(selectedLayer.id, {
                        system: e.target.value as LatticeSystem,
                      })
                    }
                    className="mt-0.5 w-full bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-xs focus:outline-none focus:border-blue-500"
                  >
                    <option value="square">
                      Square — a = b, γ = 90° (e.g. fcc(100))
                    </option>
                    <option value="rectangular">
                      Rectangular — a ≠ b, γ = 90° (e.g. fcc(110))
                    </option>
                    <option value="hexagonal">
                      Hexagonal — a = b, γ = 120° (e.g. fcc(111), hcp)
                    </option>
                    <option value="oblique">Oblique — a, b, γ all free</option>
                    <option value="centered_rect">
                      Centered Rectangular — c(n×m) structures
                    </option>
                  </select>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <input
                      type="text"
                      value={selectedLayer.a}
                      onChange={(e) => {
                        const val = e.target.value;
                        if (val === "") return; // Don't update state if empty, or set to a default
                        const v = parseFloat(val);
                        if (!isNaN(v)) upd(selectedLayer.id, { a: v });
                      }}
                  ...
                  {["rectangular", "oblique", "centered_rect"].includes(
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={selectedLayer.b}
                        onChange={(e) => {
                          const val = e.target.value;
                          if (val === "") return;
                          const v = parseFloat(val);
                          if (!isNaN(v)) upd(selectedLayer.id, { b: v });
                        }}
                        className="mt-0.5 w-full bg-gray-800 border border-gray-700 rounded px-1 py-1.5 text-xs text-center focus:outline-none focus:border-blue-500"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (val === "") return; // Don't update state if empty, or set to a default
                          const v = parseFloat(val);
                          if (!isNaN(v)) upd(selectedLayer.id, { gamma: v });
                        }}
                        className="mt-0.5 w-full bg-gray-800 border border-gray-700 rounded px-1 py-1.5 text-xs text-center focus:outline-none focus:border-blue-500"
                      />
                    </div>
            ...
            {selectedLayer.mode === "superlattice" && (
              <MatrixInput
                matrix={selectedLayer.matrix}
                onChange={(m) => upd(selectedLayer.id, { matrix: m })}
              />
            )}
            ...
            <div>
              ...
              <div className="flex gap-1">
                {[0, 30, 45, 60, 90, 120, 180].map((deg) => (
                  <button
                    key={deg}
                    onClick={() => upd(selectedLayer.id, { rotationDeg: deg })}
            ...
            <div className="border-t border-gray-800 pt-3 space-y-3">
              ...
              <Slider
                label="h,k Range"
                value={selectedLayer.hkRange}
                min={1}
                max={20}
                step={1}
                display={`±${selectedLayer.hkRange}`}
                onChange={(v) =>
                  upd(selectedLayer.id, { hkRange: Math.round(v) })
                }
              />
            </div>
            ...
            <button
              onClick={() =>
                upd(selectedLayer.id, { visible: !selectedLayer.visible })
              }
            ...
            <p className="text-xs text-gray-600 text-center">
              ...
              {selectedIdx === 0
                ? " (substrate — reference for superlattice layers)"
                : ""}
            </p>
          </div>
        )}
        ...
        <div className="border-t border-gray-800 px-4 py-3 flex-shrink-0 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <label className="flex items-center gap-1.5 text-xs cursor-pointer select-none">
              <input
                type="checkbox"
                checked={showEwald}
                onChange={(e) => setShowEwald(e.target.checked)}
          ...
          <div className="flex justify-between text-xs text-gray-600">
            ...
            <span className="text-gray-500">
              k‖ = {(0.5123 * Math.sqrt(energy)).toFixed(2)} Å⁻¹
            </span>
            <span>500 eV</span>
          </div>
        </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="flex-1 flex min-h-0">
          {(viewMode === "leed" || viewMode === "split") && (
            <div
              className={`${viewMode === "split" ? "w-1/2 border-r border-gray-800" : "w-full"} relative`}
            >
              {viewMode === "split" && (
